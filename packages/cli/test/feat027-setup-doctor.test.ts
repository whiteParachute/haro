...
describe('guided setup and doctor remediation [FEAT-027]', () => {
  ...
  it('setup --check --json returns staged results with issue contract and next actions', async () => {
    ...
    expect(json.nextActions).toEqual(expect.arrayContaining(['haro doctor', 'export OPENAI_API_KEY=<your-key>', 'codex login --device-auth', 'use AgentDock/ModelHub provider management for standalone provider checks']));
  ...
  it('smoke-test marks offline dry-run passed when provider secret is missing', async () => {
    ...
    expect(smoke.nextActions).toEqual(expect.arrayContaining(['export OPENAI_API_KEY=<your-key>', 'codex login --device-auth', 'use AgentDock/ModelHub provider management for standalone provider checks']));
  });
