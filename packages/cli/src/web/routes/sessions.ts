...
export function createSessionsRoute(runtime: WebRuntime): Hono<ApiKeyAuthEnv> {
  ...
  route.delete('/:id', (c) => {
    ...
    try {
      db.exec('BEGIN');
      try {
        db.prepare(`DELETE FROM session_events WHERE session_id = ?`).run(c.req.param('id'));
        const result = db.prepare(`DELETE FROM sessions WHERE id = ?`).run(c.req.param('id'));
        db.exec('COMMIT');
        if (result.changes === 0) return c.json({ error: 'Session not found' }, 404);
        return c.json({ success: true, data: { deleted: true, sessionId: c.req.param('id') } });
      } catch (error) {
        db.exec('ROLLBACK');
        throw error;
      }
