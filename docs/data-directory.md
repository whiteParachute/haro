...
├── assets/
│   ├── manifests/           # sidecar asset manifest；canonical registry read model
│   ├── events/              # AssetEvent contract JSON；`haro_asset_query` 解析 latest event
│   ├── snapshots/           # gated apply 前快照，后续 FEAT-047 补齐
...
channels:
  cli:
    enabled: true
  # web channel removed in sidecar-era; Haro Web only reviews approval requests.
  feishu:
    enabled: false
    appId: "${FEISHU_APP_ID}"
...
FEAT-031 Web Channel 已在 2026-05-14 sidecar cleanup 中下线：`~/.haro/channels/web/`、`web_messages`、Dashboard Chat/WebSocket streaming 不再属于当前数据目录 contract。Haro Web 只读取/写入 `~/.haro/evolution/approval-requests`、`approval-decisions` 和相关 proposal refs。
