...
| Phase D | Scheduled sidecar      | `connect agent-dock` + `observe --since last` + `propose --auto-dry-run` + `validate --pending` 已落地；propose/validate 已显式报告损坏 JSON 并原子写 artifact；下一步补 status/doctor |
