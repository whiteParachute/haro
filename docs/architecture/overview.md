...
| Phase F | Gated apply L0/L1      | gate preflight + metadata snapshot 已落地：`haro snapshot --proposal-id` 写 snapshot/rollback artifacts；`haro apply --proposal-id` 校验 proposal/validation/L0-L1/snapshot/rollback refs，缺 refs 时生成 metadata refs，ready 时写 application record；暂不修改资产内容 |
