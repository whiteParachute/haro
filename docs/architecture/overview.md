...
| Phase D | Scheduled sidecar      | `connect agent-dock` + `observe --since last` + `propose --auto-dry-run` 已落地；propose 已显式报告损坏 JSON 并原子写 proposal；下一步补 validate/status/doctor |
