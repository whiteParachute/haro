...
| Phase E | Signal intake + asset registry | `FrontierSignal` schema + `haro intake frontier --source-config` + `propose --include-frontier` 已落地，active frontier signals 会作为 dry-run proposal evidence；file-backed asset registry 第一段已落地，`haro_asset_query` 读取 sidecar registry manifests/events |
| Phase F | Gated apply L0/L1      | proposal + validation + snapshot + rollback gate |

## 架构变更记录

- **2026-05-14**：Phase E asset registry adapter 第一段落地：新增 `~/.haro/assets/manifests` + `~/.haro/assets/events` file-backed sidecar registry，`haro_asset_query` 改为读取 sidecar registry，不再读取旧 core EvolutionAssetRegistry；仍不读写 Haro-owned memory 或 `aria-memory-vault`。
