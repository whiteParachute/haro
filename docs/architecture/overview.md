...
| Phase F | Gated apply L0/L1      | gate preflight + snapshot 已落地：`haro snapshot --proposal-id` 写 snapshot/rollback artifacts，并为 allowlisted sidecar-local L0 内容生成 `snapshot-content`；`haro apply --proposal-id` 校验 proposal/validation/L0-L1/snapshot/rollback refs，缺 refs 时生成 refs，ready 时写 application record；暂不修改目标资产 |
...
- **2026-05-14**：Phase F snapshot/rollback metadata 落地：新增 `AssetSnapshotRecord` / `RollbackRecord` contract 与 `haro snapshot --proposal-id`，`haro apply --proposal-id` 可在缺 refs 时生成 metadata-only snapshot/rollback artifacts；status/doctor 纳入 snapshots/rollbacks/applications 计数；仍不执行内容 apply、不写 `applied` event、不写 memory。
