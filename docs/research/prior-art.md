...
```
aria-memory ──→ 历史 Haro MemoryFabric 借鉴；sidecar baseline 改为 AgentDock-owned memory
...
- Per-Agent 私有记忆：历史 Haro-owned 目录；sidecar baseline 下由 AgentDock memory 管理

**历史备注**：Haro-owned Memory Fabric 已从 sidecar baseline 删除；memory 由 AgentDock 侧提供。
...
- Zod 配置模式直接复用（改为 YAML 格式）
 - Channel / 配置分层 / 目录组织可继续参考，但不再作为当前正式 Provider 接入方案

**集成位置**：`docs/modules/channel-layer.md`，`docs/data-directory.md`
