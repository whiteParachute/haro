...
- Phase D 核心闭环完成；Phase E frontier evidence 主链路已接入 proposal；sidecar asset registry adapter 已接入 propose/validate；Phase F gated apply 前置骨架已启动，`haro snapshot --proposal-id` 生成 metadata-only snapshot/rollback artifacts，`haro apply --proposal-id` 会先做 L0/L1 gate preflight。
...
- 下一步：继续补 target-specific content snapshot、真实 apply executor、applied asset event 与 rollback executor。
...
- 已实现 metadata-only snapshot/rollback：`haro snapshot --proposal-id <id>` 写 `~/.haro/evolution/snapshots/*` 与 `~/.haro/evolution/rollbacks/*`；`haro apply --proposal-id` 缺 refs 时会先生成这些 refs。
- Gate 通过时写 `~/.haro/evolution/applications/<application-id>.json` ready record，`applied=false`；当前仍不修改目标资产、不写 `applied` asset event、不读取或写入 memory。
- 后续补 target-specific content snapshot、真实 apply executor、applied asset event、rollback executor。
