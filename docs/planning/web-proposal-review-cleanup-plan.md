...
当前 sidecar 分支已经明确：Haro 只作为 AgentDock 的 self-evolution sidecar，通过 MCP、AgentDock 定时任务和 AgentDock skills/agent 编排接入，不再承担通用 agent runtime / workbench。

因此历史 Web Dashboard 中的通用控制面需要清理。Haro 仍可以保留一个极小 Web 端，但职责只限于 **review Haro 自己生成的 proposal / approval request**，类似 issue 页面。
