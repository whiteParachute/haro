...
| `web` | `@haro/channel-web`（builtin） | 复用 Web API HTTP + WebSocket | Phase 1.5 (FEAT-031) | Dashboard Chat 入口；不开监听端口（端口由 web-api 提供）；可 disable，不可 remove |
...
| `web` | 原生支持 | 通过 Web API `/ws` 推 `channels.web.event`（kind=`agent` 携带 delta） |
