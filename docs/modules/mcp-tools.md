...
sidecar baseline 下，`haro mcp` 默认注册 AgentDock-facing Haro tools：`haro_observe`、`haro_propose`、`haro_validate`、`haro_asset_query`，以及 `haro_run_daily_workflow`。前四个保持 read-only/dry-run；`haro_run_daily_workflow` 只写 Haro sidecar-owned artifacts（observations/proposals/validations/approval-requests/assets），不写 AgentDock、不 apply。只有显式 `haro mcp --enable-gated-write` 时，才额外注册 `haro_apply` / `haro_rollback`；这两个工具只接受 proposal/application id 并复用 CLI gated apply/rollback，不接受自由文本 patch。`haro_observe` 的 source 选择规则是：
