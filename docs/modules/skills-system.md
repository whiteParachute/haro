...
Haro 的 Skills 格式**直接兼容 Codex 的 skill 格式**（`SKILL.md` + frontmatter）。

Codex runtime 发布使用同一份 canonical 预装 skill，避免 Haro 内置 skill 与 Codex home 漂移：

```bash
haro skills sync-runtime --skill metabolism --runtime codex
```

- 默认 `metabolism` 路径会成对同步 `eat / shit`，确保知识摄入和代谢清理能力同时进入 `$CODEX_HOME/skills`。
- 目标 runtime 中已有内容不同的 skill 时默认 fail-fast；只有显式 `--overwrite` 才会在保留备份后覆盖。
- `shit` 的 Codex runtime 文档是 FEAT-011 Haro `shit` 命令的安全包装层：先 dry-run、再显式确认，通过 Haro archive / rollback 路径变更状态，不直接执行文件清理。
...
Phase 1 起，skill proposal、promote、disable、archive 都要同步写入 Evolution Asset Registry：

- 新 proposal：`kind = 'skill'`，`status = 'proposed'`，记录 sourceRef、contentHash、四问验证结果。
- promote/install：asset 版本递增，`status = 'active'`，写入 `promoted` 事件。
- disable/uninstall/archive：不直接丢失来源，写入 `archived` 或 `superseded` 事件，rollback 走 asset event 链。
- Skill memory：成功/失败使用模式不写散文件，挂到对应 skill asset 的 `assetRef`。

实现落地边界（FEAT-022）：

- `SkillsManager.installFromPath/installFromGit` 使用稳定 `skill:<id>` asset id 记录安装后的 active skill；`contentHash` 只表示当前 `SKILL.md` 内容，不作为身份。
- `enable/disable` 分别追加 `enabled` / `disabled` audit event；状态集合不新增 `disabled`，禁用事实通过事件 metadata 表达。
- `prepareTask()` 和 `invokeCommandSkill()` 在实际触发 skill 时追加 `used` event，和原有 `usage.sqlite` 计数互补。
...
| Phase 1 | Skill marketplace 雏形 + Agent 级 skill 绑定 + Evolution Asset Registry 同步 |
