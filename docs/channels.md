...
| `web` | 内置 | HTTP + WebSocket（Web API） | structured `StreamEvent` 通过 `channels.web.event` 推送，legacy delta 并行兼容 | Phase 1.5 (FEAT-031 / FEAT-034) | 默认启用；Dashboard 走 `/api/v1/channels/web/*`；禁用后 Dashboard 进入只读 |
...
Web Channel 不开新 WebSocket 端点，复用现有 `/ws`：客户端通过 `subscribe { channel: 'channels:web' }` 或带 sessionId 的 sessions 订阅，服务端推 `channels.web.event` 消息，载荷 kind 包括 `message`（用户消息已持久化）/ `stream`（FEAT-034 structured `StreamEvent`：message、thinking、tool、hook、usage、status、error 分轨）/ `agent`（legacy delta 迁移兼容）/ `session.update`。新 Dashboard Chat store 优先消费 `stream`，并对同一 message 的 legacy payload 去重。
