...
| Phase F: Gated Apply L0/L1 | 已启动 gate + metadata snapshot | 在 validation gate 后执行低风险变更 | `ApplicationRecord` / `AssetSnapshotRecord` / `RollbackRecord` contract、`haro snapshot --proposal-id`、`haro apply --proposal-id` gate preflight；真实 apply/rollback executor 待实现 | Haro 自有 metadata record，不改目标资产 |
...
当前已实现 CLI gate preflight 与 metadata snapshot：`haro snapshot --proposal-id <id>` 写 `~/.haro/evolution/snapshots/*` 和 `rollbacks/*`；`haro apply --proposal-id <id>` 会拒绝 L2/L3、未验证 proposal、`applyEligible=false` 的请求，缺 snapshot/rollback ref 时先生成 metadata refs；gate 通过时只写 `~/.haro/evolution/applications/*` ready record，`applied=false`，暂不修改资产内容、暂不写 `applied` asset event。
