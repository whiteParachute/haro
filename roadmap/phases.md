...
| FEAT-031 | Web Channel（Web UI 作为 IM channel：对话、历史、文件） — done 2026-05-07（spec close，AC4 publish-bridge 回归测试 + spec §5.2/D4 与 FEAT-035 D8 对齐） | 1–1.5 周 | P0 |
| FEAT-032 | MCP 工具层 + 4 核心工具（send_message / memory_query / memory_remember / schedule_task） — done 2026-05-06 | 1.5–2 周 | P0 |
| FEAT-033 | Cron 任务最小版（cron + 一次性，复用现有 session 上下文） — done 2026-05-06 | 1 周 | P1 |
| FEAT-034 | 流式 UX 升级（thinking 折叠、tool timeline、Hook 状态、GFM/lightbox）— done 2026-05-07 | 1.5–2 周 | P1 |
| FEAT-035 | Memory Fabric v2（Aria-Memory 文件存储对齐：删除 SQLite/FTS5，统一文件搜索） | 1–1.5 周 | P0 |
...
- [x] Web channel 可在浏览器对话、查看历史、上传/下载文件（FEAT-031 done 2026-05-07；REST `/api/v1/channels/web/*` + WS `channels:web` 订阅 + 复合 cursor 分页 + 禁用后只读 + 路径遍历 / URL-encoded 防护 + ownerUserId=null 仅 admin 可读）
...
- **2026-05-07**：FEAT-034 流式 UX 升级 done — `@haro/core/stream` 提供 12 类 `StreamEvent` + `AgentEvent` 翻译器；Web Channel WS 新增 structured `kind='stream'` envelope 并保留 legacy `agent` envelope；Dashboard Chat 接入 `MessageStream` / `MarkdownRenderer` / `ThinkingPanel` / `ToolTimeline` / `ImageLightbox`，支持 GFM、代码高亮/unknown fallback、多图 lightbox、react-window 虚拟化；Codex 对抗性 review 追加 structured/legacy 去重、CodeBlock fallback 与 design token 状态色回归。
- **2026-05-06**：FEAT-032 MCP 工具层实现交付 — 新建 `packages/mcp-tools/` 包：`McpServer`（stdio + JSON-RPC，dependency-free 内置实现）+ `ToolRegistry`（per-tool timeout 强制、permission/audit 守门链）+ 4 个工具（`send_message` / `memory_query` / `memory_remember` / `schedule_task`）。新增 `tool_invocation_log` 表（params 仅 sha256 hash，绝不入原文）+ AgentRunner `mcpSessionFactory` 钩子（per-session subprocess，SIGTERM 5s 后 SIGKILL）+ `services.mcp.listInvocations` 只读视图。
- **2026-05-06**：FEAT-031 Web Channel 实现交付 — 新建 `packages/channel-web/`、`/api/v1/channels/web/*` 路由族、CLI 注册 Web 为 builtin enabled-by-default、Dashboard ChatPage 重写为 Web Channel 客户端。
- **2026-05-07**：FEAT-031 spec close to done — AC4 加 publish-bridge 回归测试；spec §5.2 移除实际不存在的 `WS .../sessions/:id/stream` 端点，明确流复用 `/ws + subscribe channels:web`；D4 与 FEAT-035 D8 对齐区分 transcript / 长时记忆边界。
- **2026-05-06**：Phase 1.5 新增 FEAT-035 Memory Fabric v2（Aria-Memory 对齐），承担 FEAT-031 D4 决议下的 SQLite/FTS5 → 文件存储改造；FEAT-031 收敛 Open Questions 为 D1–D4。
