...
| FEAT-031 | Web Channel（Web UI 作为 IM channel：对话、历史、文件） — done 2026-05-06（spec 仍 draft 等 FEAT-032 send_message 集成） | 1–1.5 周 | P0 |
...
- [x] Web channel 可在浏览器对话、查看历史、上传/下载文件（FEAT-031 实现交付 2026-05-06；spec 仍 draft 等 FEAT-032 send_message 集成）
...
- **2026-05-06**：FEAT-031 Web Channel 实现交付 — 新建 `packages/channel-web/`、`/api/v1/channels/web/*` 路由族、CLI 注册 Web 为 builtin enabled-by-default、Dashboard ChatPage 重写为 Web Channel 客户端。spec 仍保持 draft 等 FEAT-032 `send_message` 集成。
