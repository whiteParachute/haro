...
| Phase 1.5: Workbench Parity | **进行中**（FEAT-031/032/033/035/038/039 done）| 日用底座补完 + 架构解耦 | Web API 解耦 + CLI 等价补完 + Web Channel + MCP 工具层 + 定时任务 + 流式 UX 升级 + Memory Fabric v2 | 人类驱动 |
...
**状态**：进行中（2026-05-01 启动；FEAT-038 + FEAT-039 + FEAT-033 + FEAT-031（实现交付）+ FEAT-032（实现交付）+ FEAT-035（核心实现）已 done，FEAT-034 仍 draft）
...
- [ ] Memory Fabric v2：MEMORY.md + 散文件存储统一，`searchMemoryFiles` 替代 FTS5 read model，迁移工具幂等（FEAT-035 draft）
