...
>
> 规划背景：[`docs/planning/redesign-2026-05-01.md`](../docs/planning/redesign-2026-05-01.md)
...
| Phase 1.5: Workbench Parity | **进行中** | 日用底座补完 + 架构解耦 | Web API 解耦 + CLI 等价补完 + Web Channel + MCP 工具层 + 定时任务 + 流式 UX 升级 | 人类驱动 |
...
**Phase 1.5 验收标准**：
- [ ] `packages/web-api` 独立可发布，前后端通过稳定 HTTP/JSON contract 通信
- [ ] CLI 命令族覆盖 Web Dashboard 所有页面的核心动作（chat/session/agent/skill/memory/logs/workflow/budget/user）
- [ ] Web channel 可在浏览器对话、查看历史、上传/下载文件
- [ ] Agent 可通过 MCP 工具发消息、查/写记忆、调度任务
- [ ] 定时任务支持 cron 表达式 + 一次性触发
- [ ] 流式 UX：thinking 可折叠、tool 调用有 timeline、Markdown 含表格/代码高亮/图片预览
...
- **2026-05-01**：原 Phase 0/1/2/3/4 重排为 Phase 0/1/1.5/2.0/2.5/3.0/3.5；Phase 4 Ecosystem 移除；Phase 1.5 新增；Phase 2/3 内部重构为感知层 / 提案层 / 受控自演化 / Agent-as-Developer 四段。详见 [`docs/planning/redesign-2026-05-01.md`](../docs/planning/redesign-2026-05-01.md)。
