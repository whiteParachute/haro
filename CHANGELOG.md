...
- FEAT-031 done — Web Channel（浏览器作为 IM 渠道）spec close。实现侧 2026-05-06 已交付（`@haro/channel-web` + `/api/v1/channels/web/*` REST + WS `subscribe channels:web` + `0600` 文件存储 + 禁用后只读模式 + 复合 cursor `(before, beforeId)` 分页），Codex 评审 6 must-fix + 4 should-fix 全修。本次 spec close 完成：（a）AC4 措辞收紧并补 web-feat031 publish-bridge 回归测试，验证 `WebChannel.send → WebSocketManager.publishWebChannelEvent` 桥接（broadcast 由 manager 单元测试覆盖；cross-package E2E 列为 known gap，owner 接受契约级覆盖）；（b）spec §5.2 删除实际不存在的 `WS .../sessions/:id/stream` 端点，明确 stream 复用 Dashboard `/ws + subscribe channels:web`；（c）D4 与 FEAT-035 D8 对齐——会话 transcript 走 Web Channel `web_messages` cursor 分页（**本期不实现** REST 关键字过滤），长时记忆才走 `searchMemoryFiles`；（d）spec §5.3 schema 与 `packages/channel-web/src/persistence/messages.ts` 实际定义对齐（补 `web_sessions` 主表）；（e）`send_message` 测试加 `OutboundMessage` payload 字段断言锁定 MCP ↔ channel 类型契约；（f）`web-feat031` 加 `beforeId` 复合 cursor REST 路径覆盖。`packages/core/src/memory/memory-fabric.ts:265-272` 注释同步去除"FEAT-031 走 searchMemoryFiles"误述。验证：core 274/274 + cli 151/151 + web-api 74/74（FEAT-031 7 → 9 用例）+ mcp-tools 56/56，`pnpm lint` clean。
