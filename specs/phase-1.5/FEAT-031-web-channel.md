...
- D4（原 Q4，历史检索）：**走 Memory Fabric，不在 Web Channel 内重起 FTS5**。Memory Fabric 整体照搬 aria-memory 的"文件存储 + 索引"模式，因此 Web Channel 历史搜索 = Memory Fabric 内的文件存储搜索；Web Channel 自身的 `web_messages` 表只承担时间序游标分页（`before=<ts>&limit=<n>`），不再单独建索引。Memory Fabric 改造由 [FEAT-035](FEAT-035-memory-fabric-aria-memory-alignment.md) 承担，不在 FEAT-031 内。
