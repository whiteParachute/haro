...
related:
  ...
  - ../phase-1.5/FEAT-035-memory-fabric-aria-memory-alignment.md
...
- D4（原 Q4，历史检索；2026-05-07 与 FEAT-035 D8 对齐后细化）：把 Web Channel 历史明确**分两类**——
  ...
  - **长时记忆历史**（"agent 跨 session 想找过去的相关知识"）：走 Memory Fabric `searchMemoryFiles`（FEAT-035 提供，aria-memory 风格 grep + MEMORY.md 索引）。Memory Fabric 改造由 [FEAT-035](FEAT-035-memory-fabric-aria-memory-alignment.md) 承担，已 done 2026-05-07。
  - 相同决议在 FEAT-035 D8 也有交叉记录。**不**在 Web Channel 内重起 FTS5 / 向量索引。

## 9. Changelog / 变更记录

- 2026-05-01: whiteParachute — 初稿（Phase 1.5 自用底座补完批次 1）
- 2026-05-06: whiteParachute — 收敛 Open Questions Q1–Q4 为 D1–D4（owner 决策：S3 不做 / 不分屏 / 不加密 / 历史搜索走 Memory Fabric aria-memory 风格）；新增 FEAT-035 承担 Memory Fabric v2 改造，related 增补 FEAT-021 / FEAT-035
