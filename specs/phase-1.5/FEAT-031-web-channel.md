...
- R3: Web Channel session ID 必须与 Web Dashboard 用户的 chat session 映射，历史实现复用 Haro MemoryFabric session 维度；sidecar baseline 由 AgentDock session/memory 提供，不引入第二套 ID 体系。
...
依赖：`@haro/channel`（协议层）、`@haro/core`（历史 MemoryFabric session 兼容层）、`better-sqlite3`、`mime-types`。
...
GET    /api/v1/channels/web/sessions/:id/messages?before=<ts>&beforeId=<id>&limit=<n>
...
实时事件流复用 Web Dashboard 既有 `/ws`：客户端 `subscribe { channel: 'channels:web' }` 后服务端在该 session 的出/入站消息上推 `{ type: 'channels.web.event', sessionId, event }`，**不**单独挂 `/api/v1/channels/web/sessions/:id/stream` 端点。这一选择对齐 G6（不引入新传输协议）；event payload 格式见 `packages/channel-web/src/stream.ts` 的 `WebChannelStreamEvent`。

### 5.3 消息历史持久化

`sessions.sqlite` schema（实际落地版本，权威定义见 [`packages/channel-web/src/persistence/messages.ts`](../../packages/channel-web/src/persistence/messages.ts)）：

```sql
CREATE TABLE web_sessions (
  session_id    TEXT PRIMARY KEY,
  title         TEXT,
  owner_user_id TEXT,                          -- null = unscoped/admin-only
...
CREATE TABLE web_messages (
  ...
  metadata    TEXT,
  created_at  INTEGER NOT NULL                 -- ms epoch (cursor-friendly)
);
...
### 5.4 与 Memory 的关系（历史 workbench）

Web Channel session 在历史 workbench 中曾与 Haro MemoryFabric `session` scope 对接。sidecar baseline 下 Web/Memory 均由 AgentDock 承载；Haro 不再把 Web Channel transcript 写入自有 MemoryFabric。
...
- AC4: FEAT-032 `send_message` MCP 工具可路由到 Web Channel session（验收点：`send_message` 工具走 `entry.channel.send(sessionId, OutboundMessage)` 通用路径，对任何 registered channel 等价；WebChannel.send 写 `web_messages` 并触发 stream event；web-api `tryWebChannel()` 把 `channel.onStream` 桥到 `WebSocketManager.publishWebChannelEvent`，再由 manager broadcast 给订阅 `channels:web` 的 WS 客户端 — 这条桥本身有回归测试，broadcast 行为由 manager 单元测试覆盖）。**已知缺口**：未补 cross-package E2E（一次调用串 send_message → real WebChannel → web_messages → WS 客户端实收），owner 接受作为契约级覆盖（对应 R4）。
...
- 集成测试：起 web-api 服务，模拟 Dashboard 发消息 → DB 写入 → WS 推送 → 历史读取 → 历史 MemoryFabric session 兼容对账。
...
- 回归：FEAT-016 Web Dashboard Chat 既有功能、FEAT-019 Channel 管理 UI、历史 MemoryFabric session scope。

## 8. Resolved Decisions / 已决议（原 Open Questions）

- D1（原 Q1，文件存储后端）：**不引入 S3/OSS adapter**。本地文件系统就够；多设备访问如果未来真要做，由那时另起 spec 评估，不在 FEAT-031 范围。
- D2（原 Q2，群聊/分屏语义）：**不做**。Web Channel 维持单 user 多 session、每个 session 1:1 对话；不支持 Dashboard 分屏，避免范围扩散。
- D3（原 Q3，文件加密）：**不做**。Phase 1.5 仅靠 0600 目录权限 + RBAC（FEAT-028）。Phase 2.0+ 是否加密由那时评估，不阻塞本期。
- D4（原 Q4，历史检索；2026-05-07 与已归档的 historical MemoryFabric v2 决策对齐后细化）：把 Web Channel 历史明确**分两类**——
  - **会话 transcript 历史**（"上下滑动看本 session 的过往消息"）：走 Web Channel 自己的 `web_messages` 表 + 复合 cursor `(before=<ts>, beforeId=<id>)` 分页；同毫秒消息靠 `beforeId` tie-break。**本期不实现** REST 层关键字过滤（`/messages` 没有 `query` 参数），需要时在客户端对单页结果做本地过滤，或后续单起 spec 加 SQL 关键字搜索。
  - **长时记忆历史**（"agent 跨 session 想找过去的相关知识"）：历史 workbench 走 Haro MemoryFabric `searchMemoryFiles`；sidecar baseline 改走 AgentDock memory。
  - 相同决议曾在 historical MemoryFabric v2 记录中交叉出现；该 spec 文件已随 sidecar 清理归档/删除。**不**在 Web Channel 内重起 FTS5 / 向量索引。

## 9. Changelog / 变更记录

- 2026-05-01: whiteParachute — 初稿（Phase 1.5 自用底座补完批次 1）
- 2026-05-06: whiteParachute — 收敛 Open Questions Q1–Q4 为 D1–D4（owner 决策：S3 不做 / 不分屏 / 不加密 / 历史搜索走 Haro MemoryFabric 兼容层）；当时新增 MemoryFabric v2 改造；sidecar 修订后该路径为历史兼容，相关 spec 已归档/删除
- 2026-05-06: whiteParachute — 实现交付：新增 `@haro/channel-web`（WebChannel + sessions.sqlite + 上传守门 + 流事件），`@haro/web-api` 注册 `/api/v1/channels/web/*` 路由族（含 RBAC + 禁用后只读模式），CLI 注册 Web 为 builtin channel 且 `haro web` 启动前 `start()`，Dashboard ChatPage 重写为 Web Channel 客户端（POST /messages + WS subscribe channels:web + 历史游标分页 + 禁用 banner）；AC1/AC2/AC3/AC5/AC6 全覆盖（AC4 等 FEAT-032 集成）。Codex 评审 6 must-fix + 4 should-fix 全修；channel-side 回包 sessionId 改为 channelSessionId，handleExternalInbound 消费 meta.agentId/providerId/modelId，`stop()` 不再关 SQLite（保留 enable→disable→enable 一致性），ownerUserId=null 仅 admin 可读，path traversal 增 URL-encoded 防护，分页改 composite cursor。spec 仍 draft 待 FEAT-032 落地后转 done。
- 2026-05-07: whiteParachute — close to done。AC4 由 FEAT-032 `send_message` 工具实现 + `WebChannel.send` 实现的两侧契约共同覆盖：MCP 工具走 `entry.channel.send(sessionId, OutboundMessage)` 通用路径（packages/mcp-tools/src/tools/send-message.ts:72，对任何 registered channel 等价），FEAT-032 send_message 7 用例覆盖路由 / 跨 channel needs-approval / target-not-found / target-disabled / invalid-params / channel.send-failure，channel-web 34 用例覆盖 `WebChannel.send` 写 `web_messages` + 流事件，web-api `web-feat031.test.ts` 7 用例覆盖 REST + RBAC + disable 后只读 + 复合 cursor 分页。已知缺口：未补 cross-package E2E（一次调用串 send_message → real WebChannel → web_messages → WS）；owner 接受作为契约级覆盖。historical MemoryFabric v2 决策（2026-05-07）已记录 Web Channel 会话历史走自身 sessions.sqlite 时间游标分页、长时记忆才走 `searchMemoryFiles` 的边界；sidecar 清理后该 spec 已归档/删除，本 spec D4 保留最终边界。status: draft → done。
