...
Phase 1.5 取**最小子集**：只做 cron + once，复用现有 session 上下文（group 模式）；isolated / interval 模式延后到 sidecar-era 视需要再做。架构上对齐 hermes 的 tick 模型而非 happyclaw 的常驻调度对象。
...
- 不实现 isolated 上下文（独立环境无 session 复用）；延后到 sidecar-era。
...
- Q1: 同时 due 的多个任务串行还是并行？倾向并行但受 FEAT-023 budget 控制；待 sidecar observation pipeline 看实际并发负载再调。
...
- Q5: tick 默认周期 60s 是否够用？sidecar observation 数据可能要求降到 30s；先 60s。
