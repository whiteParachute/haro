...
- 2026-05-02: whiteParachute — 批次 3 Codex adversarial review 修复（round 2）：(1) failing `provider/channel/gateway doctor --json` 不再误把 `report.ok=false` 包成 `{ok:true,data}` 写到 stdout；新 helper `renderJsonDiagnostic` 在失败分支写 `CliErrorEnvelope`（`PROVIDER_DOCTOR_FAILED` / `CHANNEL_DOCTOR_FAILED` / `GATEWAY_DOCTOR_FAILED`）到 stderr，stdout 留空，配合非零 exit。(2) REPL `/budget` 不再用 `listWorkflowBudgets({limit:1})` 的全局最新行（共享 root 上的 gateway / 飞书 turn 会污染该判定）；改成 `executeTask` 返回 `workflowId`、`ReplState.lastWorkflowId` 显式记录、`/budget` 走 `getWorkflowBudget(workflowId)`。新增 `packages/cli/test/feat039-batch3-fixes.test.ts` 三条回归测试。491 测试全过。
