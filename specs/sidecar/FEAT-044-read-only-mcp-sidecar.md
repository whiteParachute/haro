...
- 默认 `haro mcp` surface 不实现 / 不注册 `haro_apply`。
- 默认 `haro mcp` surface 不实现 / 不注册 `haro_rollback`。
- FEAT-047 的 gated-write MCP bridge 只能通过显式 `haro mcp --enable-gated-write` 开启，不改变本 spec 的默认 read-only 验收。
...
- AC5: 给定默认 `haro mcp` 的 `tools/list`，不应出现 `haro_apply`、`haro_rollback`、`memory_query`、`memory_remember` 或 `send_message`。（对应 R7/R10）
