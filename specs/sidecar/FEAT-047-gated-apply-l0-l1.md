...
- R11: `haro snapshot --proposal-id` 可为 L0/L1 proposal 生成 metadata-only `AssetSnapshotRecord` 与 `RollbackRecord`；该 metadata 不等同于 target-specific content snapshot，真实内容 apply 前仍需 executor 侧补齐目标内容快照。
...
- `haro snapshot --proposal-id <id> --json` 已实现 metadata-only snapshot/rollback artifact 生成，写入 `~/.haro/evolution/snapshots/*` 与 `~/.haro/evolution/rollbacks/*`。
...
- Gate 缺 snapshot/rollback refs 时会先生成 metadata-only refs。
- Gate 通过后写 `~/.haro/evolution/applications/<application-id>.json` ready record，`applied=false`。
- 当前不创建 target-specific content snapshot、不执行 changeSet、不写 `applied` asset event、不读写 memory。
...
- AC2.3: 给定 eligible L0 proposal 但 proposal rollback refs 为空，当调用 `haro apply --proposal-id` 时，应先生成 metadata-only snapshot/rollback refs，再写 ready application record；仍不得修改目标资产。（对应 R10/R11）
...
- CLI gate：eligible L0 proposal 缺 refs 时自动生成 metadata snapshot/rollback，再写 ready application record。
