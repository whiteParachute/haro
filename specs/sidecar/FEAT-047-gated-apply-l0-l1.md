...
- 当前不执行 changeSet、不写 `applied` asset event、不读写 memory。
...
- AC2.3: 给定 eligible L0 proposal 但 proposal rollback refs 为空，当调用 `haro apply --proposal-id` 时，应先生成 snapshot/rollback refs，再写 ready application record；仍不得修改目标资产。（对应 R10/R11）
...
- CLI gate：eligible L0 proposal 缺 refs 时自动生成 snapshot/rollback，再写 ready application record。
