...
- `haro rollback --application-id <id> --json` 已实现 sidecar-local L0 rollback executor：`restore-latest-event` 只恢复 sidecar-owned `snapshot-content`，`delete-created-asset` 删除 apply 创建的 current content；成功后把 application record 更新为 `rolled-back/applied=false`，并写 `rolled-back` asset event。
- 当前不支持 L1 executor、不支持 apply delete/archive op、不读写 memory。
