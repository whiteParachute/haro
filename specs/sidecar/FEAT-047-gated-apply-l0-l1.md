...
- R2.1: 启动阶段 proposal schema 默认 `humanReviewRequired=true`；若 proposal 缺少 `humanApprovalRefs` 且没有 approve decision 可消费，`haro apply` / `haro_apply` 必须返回 `HUMAN_REVIEW_REQUIRED`，不得生成 snapshot、application 或 asset event。
- R2.2: 若存在 reject 或 request-changes decision，`haro apply` / `haro_apply` 必须 fail closed：reject 返回 `APPROVAL_REJECTED`，request-changes 返回 `CHANGES_REQUESTED` 并包含 reviewer direction。
...
- R13: Phase F sidecar-local apply 只允许从 `~/.haro/evolution/proposal-content/<proposal-id>/` 读取拟应用内容，并只写入 allowlisted `~/.haro/assets/current/<kind>/`；必须写 `ApplicationRecord(status=applied)` 与 `applied` asset event，仍不得读写 memory。
...
- R15: Phase F sidecar-local rollback 第一段只允许对 `ApplicationRecord(status=applied)` 执行 `haro rollback --application-id`，基于同 application 绑定的 snapshot/rollback artifact 恢复 `snapshot-content` 或删除 apply 创建的 sidecar-local 内容；必须写 `ApplicationRecord(status=rolled-back)` 与 `rolled-back` asset event，仍不得读写 memory。
- R16: Phase F sidecar-local L1 executor 第一段只允许写 Haro sidecar-owned `~/.haro/assets/current/{skill,runner-profile,schedule-config,routing-rule}/`，并复用同一套 snapshot/apply/rollback gate；不得直接写 AgentDock skill/profile/task internals。
- R17: `haro mcp` 默认必须保持 FEAT-044 read-only surface；只有显式 `--enable-gated-write` 时才注册 `haro_apply` / `haro_rollback`，且二者必须委托同一套 CLI apply/rollback gate。
...
haro_apply({ proposalId })
  -> load proposal
  -> sync latest approval decision artifact
  -> load validation report
  -> check level in L0/L1
  -> block reject / request-changes decisions
  -> check humanApprovalRefs when humanReviewRequired=true
  -> check applyEligible
  -> check rollbackReady
  -> create snapshot
  -> apply change set
...
| 缺少 approval ref | `HUMAN_REVIEW_REQUIRED` |
| 人审驳回 | `APPROVAL_REJECTED` |
| 人审要求修改 | `CHANGES_REQUESTED` |
...
- `haro apply --proposal-id <id> --json` 已实现 gate preflight + sidecar-local L0/L1 apply executor。
- Gate 已覆盖 proposal existence、L2/L3 direct apply forbidden、L0/L1 target allowlist、`proposal.status=validated`、validation report、approval decision 同步、`humanReviewRequired` / `humanApprovalRefs`、`applyEligible=true`、`rollbackReady=true`、snapshot ref、rollback ref。
...
- AC2.0: 给定 validated + applyEligible 但缺少 `humanApprovalRefs` 的 proposal，当调用 `haro_apply` / `haro apply --proposal-id` 时，应返回 `HUMAN_REVIEW_REQUIRED`，且不生成 snapshot/rollback/application/asset event。（对应 R2.1）
- AC2.0a: 给定 approve decision artifact 但 proposal 尚未有 `humanApprovalRefs`，当调用 `haro apply --proposal-id` 时，应先同步 human approval evidence，再进入后续 apply gate。（对应 R2.1）
- AC2.0b: 给定 reject/request-changes decision artifact，当调用 `haro apply --proposal-id` 时，应分别返回 `APPROVAL_REJECTED` / `CHANGES_REQUESTED`，且不生成 application。（对应 R2.2）
...
- AC2.7: 给定已 apply 的 L0 `prompt` / `mcp-tool-config` application 且 snapshot-content 存在，当调用 `haro rollback --application-id` 时，应恢复 apply 前内容、更新 application record 为 `rolled-back`、写 `rolled-back` asset event，且不读写 memory。（对应 R15）
- AC2.8: 给定 apply 前不存在的 L0 sidecar-local asset，当调用 `haro rollback --application-id` 时，应删除 apply 创建的 current content、更新 application record 为 `rolled-back`、写 `rolled-back` asset event。（对应 R15）
- AC2.9: 给定 eligible L1 `runner-profile` proposal 且 sidecar-local 当前内容存在，当调用 `haro apply --proposal-id` 再调用 `haro rollback --application-id` 时，应先写入 L1 current content 和 `applied` event，再恢复 snapshot-content 并写 `rolled-back` event，且不读写 memory。（对应 R16）
...
- AC6: 给定未加 `--enable-gated-write` 的 `haro mcp`，`tools/list` 不应出现 `haro_apply` / `haro_rollback`；给定显式加该 flag 的 `haro mcp`，`tools/list` 应出现这两个工具，调用缺失 proposal 的 `haro_apply` 应返回 blocking gate result 而不是 MCP server error。（对应 R17）
...
- CLI gate：需要人审但缺少 `humanApprovalRefs` 的 proposal 返回 `HUMAN_REVIEW_REQUIRED`，不写 snapshot/rollback/application/asset event。
...
- CLI rollback：已 applied 的 L0 prompt application 可从 `snapshot-content` 恢复旧内容，写 `rolled-back` application record 与 asset event，不创建 memory。
...
- 2026-05-14: Haro — 完成 Phase F 第五段 sidecar-local rollback executor 第一段：`haro rollback --application-id` 可恢复 snapshot-content 或删除 apply 创建的 L0 prompt/mcp-tool-config current content，记录 rolled-back application/asset event；仍不读写 memory。
- 2026-05-14: Haro — 完成 Phase F 第六段 sidecar-local L1 executor 第一段：`skill` / `runner-profile` / `schedule-config` / `routing-rule` 支持同一套 sidecar-owned snapshot/apply/rollback content flow；仍不直接写 AgentDock internals，不读写 memory。
- 2026-05-14: Haro — 完成 Phase F 第七段 gated-write MCP bridge：`haro mcp` 默认 read-only，显式 `--enable-gated-write` 才注册 `haro_apply` / `haro_rollback`，并复用 CLI gate；不接受自由文本 patch，不创建 memory。
- 2026-05-14: Haro — 启动阶段补强 human review gate：proposal schema 默认 `humanReviewRequired=true`，自动 proposal 显式空 approval refs；`haro apply` 对缺少 approval ref 的 proposal 返回 `HUMAN_REVIEW_REQUIRED`，不生成 snapshot/application。
- 2026-05-08: Haro — 初稿。
