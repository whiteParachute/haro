...
Haro 的后台 observe/propose/validate 不应依赖普通聊天上下文。因此 Haro 应提供无交互 CLI，让 AgentDock workspace/agent 可以通过 `haro mcp` 或 CLI helper 周期触发；AgentDock task/script 只作为可选部署兜底。
...
- G5: 所有后台命令可被 AgentDock workspace/agent 编排或可选 script task 安全执行。
...
AgentDock workspace / agent
  -> Haro MCP or CLI helper
  -> AgentDock HTTP observation source read-only
  -> ~/.haro/evolution/*
  -> ~/.haro/assets/* asset events
  -> cursor update
```
...
- 集成测试：fake source observe → propose → validate，并检查 proposed/validated asset events 与 manifest latest status。
