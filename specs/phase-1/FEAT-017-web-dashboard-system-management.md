...
- 不实现 Memory / Skills 页面（属于 FEAT-024）或 Provider 统计页面（属于 FEAT-025）
...
- 2026-04-25: done — FEAT-017 收尾完成，status: in-progress → done。验证：`pnpm -F @haro/cli test -- web-feat017.test.ts`（实际执行 CLI 包 7 个测试文件 / 77 tests，全过）和 `pnpm -F @haro/web test -- feat017.test.tsx`（实际执行 Web 包 3 个测试文件 / 10 tests，全过）。真实 provider 连通/healthCheck 集成测试当前没有独立 harness，按 owner 指示跳过；现有覆盖保留 provider 分组/read-model contract，后续 provider 统计与深度监控归 FEAT-025。
