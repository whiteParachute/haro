...
- `eat` 可以生成 proposal bundle，但 proposal、promote、install 之间缺少统一资产身份；早期 Memory 写入路径已在 sidecar baseline 中退为历史兼容。
- `shit` 可以 archive 和 rollback，但 archive 与原始 skill/prompt/rule 的版本关系不够显式。
- Skills、prompts、routing rules、mcp 配置与 archive 分散在不同目录，后续 Evolution Engine 难以判断“哪个版本来自哪个 signal、被谁使用、是否有效”。
...
- G1: 给 skill、prompt、routing rule、mcp、archive 建立统一资产身份。
- G2: 记录资产的版本、状态、来源、内容 hash 和审计事件。
- G3: 让 eat/shit、Skills 子系统通过 asset id 串联生命周期；sidecar 修订后 memory 不再作为 Haro asset kind。
...
- 不做跨机器资产同步或团队市场（原 Phase 4 ecosystem 已在 2026-05-01 重设计中移除，Haro 定位为单实例自用工具）。

## 4. Requirements / 需求项

- R1: 系统必须提供 `EvolutionAsset` 与 `EvolutionAssetEvent` 数据模型，覆盖 skill、prompt、routing-rule、mcp、archive。
...
- R8: 历史兼容：旧 MemoryFabric entry 可保留 `assetRef` 供追溯；sidecar baseline 不再创建 memory asset。
...
```typescript
type EvolutionAssetKind = 'skill' | 'prompt' | 'routing-rule' | 'mcp' | 'archive';
...
- AC6: 历史兼容：旧 MemoryFabric entry 带 assetRef 时仍可反查；sidecar baseline 只保存 AgentDock observation refs。
...
- Memory 兼容回归：旧 memory entry 的 assetRef 可查询；sidecar baseline 不再创建 memory asset。
...
- 2026-04-25: Codex — 实现完成并标记 done：落地 core Evolution Asset Registry API、SQLite asset/event audit tables、eat/shit/Skills/历史 MemoryFabric assetRef 接入、文档同步和 AC1-AC8 测试覆盖。验证通过：`pnpm lint`、`pnpm test`、`pnpm build`、`pnpm smoke`。实现提交并推送：`fbcc43d`。
- 2026-05-08: Codex — sidecar baseline 修订：`memory` 不再是 Haro-owned `EvolutionAssetKind`，旧 MemoryFabric `assetRef` 只保留历史兼容与追溯语义。
