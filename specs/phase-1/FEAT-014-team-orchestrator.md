...
`RoutingDecision` 与 `ScenarioWorkflow`，其中 team 路径保留
`executionMode = team`、`orchestrationMode = parallel / debate / pipeline / hub-spoke`、
`workflowId`、`nodes`、`leafSessionRefs` 与 `CheckpointStore` 边界。

FEAT-014 已补齐这一断层：CLI release 路径在收到合法 team workflow 时直接调用
`TeamOrchestrator.executeWorkflow()`，由 Team Orchestrator 展开 fork、调度 leaf `AgentRunner`、生成
`MergeEnvelope`，并在 fork / leaf terminal / merge 阶段写入可恢复 checkpoint。FEAT-014 的任务不是重做路由，
而是作为 Router 之后的 team runtime control plane，承接 team workflow 的执行协议、merge 语义与 checkpoint
恢复边界。
...
- 不重做 FEAT-013 的 `Scenario Router` scene taxonomy、routing matrix 或 provider selection 逻辑。
- 不修改 `specs/team-orchestration-protocol.md` 或 Phase 0 单 Agent runtime 语义。
...
- R23: FEAT-014 release 路径必须保持小而可回滚的实现面：CLI 只负责把 Router 产物分派到 Team
  Orchestrator，Team Orchestrator 只组织 `AgentRunner` leaf attempt，不引入第二套 leaf executor 或新依赖。
...
### 5.11 与 CLI release 路径的关系

当前 CLI 在 team mode 下的行为是：

1. Router 正常产出 team `RoutingDecision` 与 `ScenarioWorkflow`。
2. CLI 将 `RoutingDecision + ScenarioWorkflow + rawContextRefs` 直接交给 `TeamOrchestrator.executeWorkflow()`。
3. Team Orchestrator 写入 fork / leaf terminal / merge checkpoint。
4. leaf branch 统一映射到 `AgentRunner.run()`；branch retry 默认隔离运行，不使用 `continueLatestSession=true`
   去按 agent/provider 查询 latest session。
5. merge 完成后 checkpoint 包含 branch ledger、leaf session refs、`MergeEnvelope` 与 `consumedBranches`。

FEAT-014 不改变 FEAT-013 的路由与 checkpoint 入口语义，只接住既有 team workflow skeleton。若未来出现非法或不支持的
workflow，运行时必须返回明确错误或显式 unsupported 边界；不得把已支持的 team workflow 静默伪装成 single-agent
fallback。
...
- AC12: CLI release 路径必须有集成测试证明 team workflow 会调用 Team Orchestrator，并在 checkpoint 中写入
  branch ledger、leaf session refs 与 merge envelope；不得保留“team warning 后 fallback single-agent”作为成功路径。
...
- **Lifecycle tests**
  ...
  - 覆盖 branch retry 不会隐式续接同 agent/provider 的普通 CLI latest session 或其他 branch session。
...
- **CLI compatibility tests**
  ...
  - CLI checkpoint 覆盖 fork、leaf terminal、merge-ready 与 merge-completed 状态，并能读取 branch ledger 与
    `MergeEnvelope`。
