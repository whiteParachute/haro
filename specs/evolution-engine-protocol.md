...
>
> 本文描述旧 Haro workbench 路线下的 Evolution Engine / OODA 协议。旧 Phase 2.0 / 2.5 spec 文件已被 `specs/sidecar/FEAT-043` 到 `FEAT-050` 替代并删除；本文仅保留为概念参考。sidecar-era 实现以 AgentDock contract、read-only MCP、scheduled CLI、asset registry adapter 和 gated apply specs 为准。
...
| Phase D | Observe / Propose scheduled | [FEAT-045](./sidecar/FEAT-045-scheduled-sidecar-cli.md) | AgentDock workspace/agent 编排触发；自动 proposal 默认 `humanReviewRequired=true` | dry-run + human-review |
...
| Phase H | Human review request | [FEAT-050](./sidecar/FEAT-050-approval-request-workflow.md) | validated proposal 转成 why/how/benefit 审批 artifact，供 AgentDock 渲染 | approval queue |
