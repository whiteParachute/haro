...
6. **资产化而非散落文件**：Phase 1 起，eat/shit 产生或影响的 skill、prompt、rule、archive 都必须进入 Evolution Asset Registry，带版本、来源和审计事件

## Phase 1 资产化调整

Phase 0 的 eat/shit 以命令流程为主：`eat` 生成 proposal bundle，`shit` 生成 archive。Phase 1 引入 Evolution Asset Registry 后，代谢产物必须同时成为可追踪资产。
...
interface EvolutionAsset {
  id: string;
  kind: 'skill' | 'prompt' | 'routing-rule' | 'mcp' | 'archive';
  name: string;
  version: number;
...
- `eat` 生成 skill/prompt/rule proposal、安装 skill 时，都必须创建或更新 asset；Memory 证据只保留 AgentDock sourceRef / observationRef。
- `shit` 归档任何对象时，都必须把对应 asset 状态置为 `archived`，并写入 rollback 事件。
- Dashboard（历史 workbench）与 Skills 子系统只通过 asset id 追踪来源，不直接猜测文件路径语义。
...
Step 3: 决策（分桶到 proposal bundle / 拒绝；Memory 证据只保留引用）
...
| 目标 | 特征 | Phase 0 行为 | 限制 |
|------|------|-------------|------|
| AgentDock memory refs | 已存在于 AgentDock 侧的事实 / 经验 | 保存 sourceRef / observationRef | 不写 Haro-owned Memory |
...
- Memory 证据：不作为 `kind`；只在 proposal/evidenceRefs 中保存 AgentDock memory observation ref。
...
| AgentDock memory refs | observation/evidence refs | 是否仍被 proposal/test 引用 |
| `all` | 以上全部 | — |
...
每次 skill 触发、rule 被加载、mcp 被调用时，Haro 增量更新此表；memory 读取由 AgentDock 侧审计，Haro 只消费 observation。
...
- 2026-05-08: Sidecar 口径修订：Memory 由 AgentDock 侧提供；Haro 不再把 memory 作为自有 asset/write target。
