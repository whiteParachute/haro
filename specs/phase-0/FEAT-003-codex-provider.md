...
Codex 是 Phase 0 当前唯一保留的正式 Provider，用于代码类任务（默认选择规则 `code-generation` 会把代码类任务优先路由给 Codex）。Codex 采用轮次制（无中途推送）、通过 `previous_response_id` 管理上下文、工具调用需要外部 MCP 处理。本 spec 覆盖 Codex 的最小可用接入。

## 2. Goals / 目标

- G1: 通过 `@openai/codex-sdk` 对接 Codex，实现 `AgentProvider` 接口
- G2: 支持跨轮次上下文延续（`previous_response_id`）
- G3: Provider 独立可插拔，与未来新增 Provider 无耦合
...
- R5: 认证读取 `process.env.OPENAI_API_KEY`；`~/.haro/config.yaml::providers.codex` 可选 `baseUrl`（企业端点覆盖）；YAML 里**不**做 `${ENV}` 插值（凭证不进配置文件）
- R6: `healthCheck()` 调用 SDK 的 models 列表接口验证 key 有效；5s 超时兜底（沿用 FEAT-002 的 race 模式）
- R7: 独立 npm 包 `@haro/provider-codex`，与其他 Provider 完全无耦合
