...
Phase 0 的核心业务路径：接收一个任务 → 选 Provider/Model → 调用 → 回写结果。本 spec 串联 FEAT-001（骨架）、FEAT-002/003（Provider）、FEAT-004（Agent 定义），产出 Haro 第一个端到端可跑的执行链路。不涉及多 Agent、Team、Scenario Router（推迟到 Phase 1）。
...
- R8: 核心选择代码中不得出现 `providerId === 'claude'` 之类特判；一切差异通过 `provider.capabilities()` 查询解决
...
- AC5: 在 Claude Provider 不可用但 Codex 可用的环境，代码类任务仍成功（选规则命中 `code-generation`，fallback 无触发）（对应 R1）
