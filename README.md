...
| Gated Apply | L0/L1 proposal + validation + snapshot + rollback 后应用 | 已实现 gate preflight、snapshot/rollback metadata，以及 prompt / mcp-tool-config 的 sidecar-local content snapshot；真实内容 apply/rollback executor 后续实现 |
