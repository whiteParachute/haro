...
| Haro Scheduled CLI | 支持 `connect agent-dock`、`observe`、`intake frontier`、`propose`、`validate`、`snapshot`、`apply`、`status`、`doctor` | `connect agent-dock` + `observe --since last` + `intake frontier --source-config` + `propose --auto-dry-run --include-frontier` + `validate --pending` + `snapshot --proposal-id` + `apply --proposal-id` + `status` + `doctor --component sidecar` 已实现；propose/validate/apply 会写入 sidecar asset events；snapshot 会为 allowlisted sidecar-local L0 内容复制 snapshot-content；apply 只支持 sidecar-local `prompt` / `mcp-tool-config` 文件，不修改 AgentDock 内部资产；status/doctor 汇总 connection/cursor/evolution store 健康，不读写 memory |
| Evolution Store | `~/.haro/evolution/*` 独立存储 observations/frontier-signals/proposals/validations/snapshots/snapshot-content/rollbacks/applications | observations/frontier-signals/proposals/validations/snapshots/snapshot-content/rollbacks 已落盘；applications 第一段 gate record 已落盘 |
...
| Gated Apply | L0/L1 proposal + validation + snapshot + rollback 后应用 | 已实现 gate preflight、snapshot/rollback metadata、prompt / mcp-tool-config 的 sidecar-local content snapshot，以及 sidecar-local apply executor；rollback executor 后续实现 |
