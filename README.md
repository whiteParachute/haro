...
| Haro MCP Server | 暴露 `haro_observe` / `haro_propose` / `haro_validate` / `haro_asset_query` | 已实现 read-only sidecar，待 AgentDock live smoke |
...
3. Read-only MCP/CLI：`haro mcp` 已实现首批 observe/propose/validate/query，默认 dry-run；下一步是 AgentDock 注册后的 live smoke。
