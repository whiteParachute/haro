...
| Haro Scheduled CLI | 支持 `connect agent-dock`、`observe`、`propose`、`validate`、`status`、`doctor` | `connect agent-dock` + `observe --since last` 已实现；其余待实现 |
| Evolution Store | `~/.haro/evolution/*` 独立存储 observations/frontier-signals/proposals/validations/applications | 待迁移 |
...
4. Scheduled sidecar：`connect agent-dock` + `observe --since last` 已落地，可由 AgentDock script task 周期触发；下一步补 `propose/validate/status/doctor`。
