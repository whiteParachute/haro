...
| Haro Scheduled CLI | 支持 `connect agent-dock`、`observe`、`propose`、`validate`、`status`、`doctor` | `connect agent-dock` + `observe --since last` + `propose --auto-dry-run` 已实现；propose 已显式暴露损坏 JSON 计数并用原子写修复确定性 proposal 文件；validate/status/doctor 待实现 |
| Evolution Store | `~/.haro/evolution/*` 独立存储 observations/frontier-signals/proposals/validations/applications | observations/proposals 第一段已落盘；frontier-signals/validations/applications 待迁移 |
...
4. Scheduled sidecar：`connect agent-dock` + `observe --since last` + `propose --auto-dry-run` 已落地，可由 AgentDock script task 周期触发；propose 的 `--limit` 限制单次 proposal 打包的 observation batch 数，并在 JSON 结果中暴露损坏 observation/proposal 计数；下一步补 `validate/status/doctor`。
