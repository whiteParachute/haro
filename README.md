...
| Haro Scheduled CLI | 支持 `connect agent-dock`、`observe`、`intake frontier`、`propose`、`validate`、`snapshot`、`apply`、`rollback`、`status`、`doctor` | `connect agent-dock` + `observe --since last` + `intake frontier --source-config` + `propose --auto-dry-run --include-frontier` + `validate --pending` + `snapshot --proposal-id` + `apply --proposal-id` + `rollback --application-id` + `status` + `doctor --component sidecar` 已实现；propose/validate/apply/rollback 会写入 sidecar asset events；snapshot 会为 allowlisted sidecar-local L0 内容复制 snapshot-content；apply/rollback 只支持 sidecar-local `prompt` / `mcp-tool-config` 文件，不修改 AgentDock 内部资产；status/doctor 汇总 connection/cursor/evolution store 健康，不读写 memory |
| Evolution Store | `~/.haro/evolution/*` 独立存储 observations/frontier-signals/proposals/validations/snapshots/snapshot-content/rollbacks/applications | observations/frontier-signals/proposals/validations/snapshots/snapshot-content/rollbacks 已落盘；applications 已记录 applied / rolled-back 状态 |
...
| Gated Apply | L0/L1 proposal + validation + snapshot + rollback 后应用 | 已实现 gate preflight、snapshot/rollback metadata、prompt / mcp-tool-config 的 sidecar-local content snapshot，以及 sidecar-local apply/rollback executor；L1 executor 后续实现 |
...
7. Gated apply：Phase F 前置骨架已启动，`haro snapshot --proposal-id <id>` 生成 snapshot/rollback artifacts，并会把 sidecar-owned `~/.haro/assets/current/{prompt,mcp-tool-config}/` 中的当前内容复制到 `~/.haro/evolution/snapshot-content/`；`haro apply --proposal-id <id>` 会执行 L0 gate、缺 refs 时先生成 snapshot/rollback refs，然后从 sidecar-owned `~/.haro/evolution/proposal-content/<proposal-id>/` 读取拟应用内容并写回 `~/.haro/assets/current/{prompt,mcp-tool-config}/`，同时写 `ApplicationRecord(status=applied)` 和 `applied` asset event；`haro rollback --application-id <id>` 可基于 rollback record 恢复 snapshot-content 或删除 apply 创建的 sidecar-local 内容，并写 `rolled-back` event；当前仍不修改 AgentDock 内部资产、不写 memory。
