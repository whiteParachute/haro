...
| Gated Apply | L0/L1 proposal + validation + snapshot + rollback 后应用 | 已实现 gate preflight、snapshot/rollback metadata、L0/L1 sidecar-local content snapshot，以及 sidecar-local apply/rollback executor；AgentDock 原生写入口后续实现 |
