...
| 组件 | 职责 | 第一版状态 |
| --- | --- | --- |
| `@haro/agentdock-contract` | AgentDock connection、observation、proposal、validation、frontier signal、asset event、snapshot、rollback、application schema | 已实现 skeleton + fake source；FrontierSignal 与 Phase F snapshot/rollback/application schema 已落地 |
...
| Haro Scheduled CLI | 支持 `connect agent-dock`、`observe`、`intake frontier`、`propose`、`validate`、`snapshot`、`apply`、`status`、`doctor` | `connect agent-dock` + `observe --since last` + `intake frontier --source-config` + `propose --auto-dry-run --include-frontier` + `validate --pending` + `snapshot --proposal-id` + `apply --proposal-id` + `status` + `doctor --component sidecar` 已实现；propose/validate 会写入 sidecar asset events；apply 仍是 gate-only，不修改目标资产；status/doctor 汇总 connection/cursor/evolution store 健康，不读写 memory |
| Evolution Store | `~/.haro/evolution/*` 独立存储 observations/frontier-signals/proposals/validations/snapshots/rollbacks/applications | observations/frontier-signals/proposals/validations/snapshots/rollbacks 已落盘；applications 第一段 gate record 已落盘 |
...
| Gated Apply | L0/L1 proposal + validation + snapshot + rollback 后应用 | 已实现 gate preflight 与 metadata-only snapshot/rollback；真实内容 apply/rollback executor 后续实现 |
...
7. Gated apply：Phase F 前置骨架已启动，`haro snapshot --proposal-id <id>` 生成 metadata-only snapshot/rollback artifacts；`haro apply --proposal-id <id>` 只做 L0/L1 gate preflight，缺 refs 时会先生成 metadata refs，gate 通过后写 ready application record；当前仍不修改资产内容、不写 `applied` asset event、不写 memory。
